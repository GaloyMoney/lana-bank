...
pub struct FiscalYear {
    ...
    #[graphql(skip)]
    pub entity: Arc<DomainFiscalYear>,
}
...
impl From<DomainFiscalYear> for FiscalYear {
    fn from(fiscal_year: DomainFiscalYear) -> Self {
        FiscalYear {
            ...
            closed_as_of: fiscal_year.closed_as_of.map(Into::into),
...
impl FiscalYear {
    ...
    pub async fn next_month_to_close(&self) -> Option<Date> {
        self.entity.next_month_to_close().map(Into::into)
    }
}
...
mutation_payload! { FiscalYearCloseMonthPayload, fiscal_year: FiscalYear }
...
mutation_payload! { FiscalYearInitPayload, fiscal_year: FiscalYear }
...
mutation_payload! { FiscalYearClosePayload, fiscal_year: FiscalYear }
...
mutation_payload! { FiscalYearOpenNextPayload, fiscal_year: FiscalYear }
