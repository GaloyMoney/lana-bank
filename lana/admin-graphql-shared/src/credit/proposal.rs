...
use super::{primitives::*, repayment::*, terms::*};
...
impl CreditFacilityProposalBase {
    ...
    async fn custodian(
        &self,
        ctx: &Context<'_>,
    ) -> async_graphql::Result<Option<crate::custody::Custodian>> {
        if let Some(custodian_id) = self.entity.custodian_id {
            let (app, _sub) = app_and_sub_from_ctx!(ctx);
            let custodians: std::collections::HashMap<_, crate::custody::Custodian> =
    ...
    async fn approval_process(
        &self,
        ctx: &Context<'_>,
    ) -> async_graphql::Result<Option<crate::governance::ApprovalProcess>> {
