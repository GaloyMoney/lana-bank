        ...
        From<core_access::UserId>,
{
    ...
    pub async fn send_partial_liquidation_initiated_notification_in_op(
        ...
        initially_estimated_to_liquidate: &old_money::Satoshis,
        initially_expected_to_receive: &old_money::UsdCents,
    ) -> Result<(), EmailError> {
        let party = self
            .customers
    ...
    pub async fn send_under_margin_call_notification_in_op(
        ...
        collateral: &old_money::Satoshis,
        outstanding_disbursed: &old_money::UsdCents,
        outstanding_interest: &old_money::UsdCents,
        price: &PriceOfOneBTC,
    ) -> Result<(), EmailError> {
        let party = self
            .customers
