...
pub struct PartialLiquidationNotificationConfig {
...
pub const PARTIAL_LIQUIDATION_NOTIFICATION_COMMAND: JobType =
    JobType::new("command.notification.partial-liquidation-notification");

pub struct PartialLiquidationNotificationJobInitializer<Perms>
where
...
impl<Perms> PartialLiquidationNotificationJobInitializer<Perms>
where
    Perms: authz::PermissionCheck,
{
...
impl<Perms> JobInitializer for PartialLiquidationNotificationJobInitializer<Perms>
where
...
{
    type Config = PartialLiquidationNotificationConfig;

    fn job_type(&self) -> JobType {
        PARTIAL_LIQUIDATION_NOTIFICATION_COMMAND
    }
    ...
    fn init(
        ...
        Ok(Box::new(PartialLiquidationNotificationJobRunner {
            config: job.config()?,
...
pub struct PartialLiquidationNotificationJobRunner<Perms>
where
    Perms: authz::PermissionCheck,
{
    config: PartialLiquidationNotificationConfig,
    email_notification: EmailNotification<Perms>,
}

#[async_trait]
impl<Perms> JobRunner for PartialLiquidationNotificationJobRunner<Perms>
where
    Perms: authz::PermissionCheck + Clone + Send + Sync + 'static,
    <<Perms as authz::PermissionCheck>::Audit as audit::AuditSvc>::Action: From<core_credit::CoreCreditAction>
        + From<core_credit_collection::CoreCreditCollectionAction>
        ...
        From<core_access::UserId>,
{
    ...
    #[tracing::instrument(
        name = "notification.partial_liquidation_notification_job.process_command",
