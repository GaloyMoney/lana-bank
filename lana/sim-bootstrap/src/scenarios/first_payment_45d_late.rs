...
pub async fn first_payment_45d_late_scenario(sub: Subject, app: &LanaApp) -> anyhow::Result<()> {
    ...
    let mut is_first_disbursal = true;
    ...
    while let Some(msg) = stream.next().await {
        match &msg.payload {
            ...
            Some(LanaEvent::Credit(CoreCreditEvent::ObligationDue {
                ...
                amount,
                ..
            })) if { cf.id == *id && amount > &UsdCents::ZERO && is_first_disbursal } => {
                n_disbursal += 1;
                is_first_disbursal = false;
