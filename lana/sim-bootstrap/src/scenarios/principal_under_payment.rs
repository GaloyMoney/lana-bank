...
pub async fn principal_under_payment_scenario(sub: Subject, app: &LanaApp) -> anyhow::Result<()> {
    ...
    let _principal_underpay_handler = tokio::task::Builder::new()
        .name("sim-bootstrap.principal-underpay-handler")
        .spawn(
            async move {
        ...
        .expect("Failed to spawn sim-bootstrap.principal-underpay-handler task");
