...
use core_credit::{
    CREDIT_ACCOUNT_SET_CATALOG, CREDIT_FACILITY_ENTITY_TYPE, CREDIT_FACILITY_PROPOSAL_ENTITY_TYPE,
    CalaAccountId, CalaAccountSetId, CollateralAccountSets, CreditFacilityActivation,
    CreditFacilityBalanceSummary, CreditFacilityId, CreditFacilityInterestAccrual,
    CreditFacilityInterestAccrualCycle, CreditFacilityInternalAccountSets,
    CreditFacilityLedgerAccountIds, CreditLedgerError, CreditLedgerOps,
    CreditSummaryAccountSetSpec, CustomerType, DisbursalId, DisbursedReceivable,
    DisbursedReceivableAccountCategory, DisbursedReceivableAccountSets,
    DisbursedReceivableAccountType, FacilityDurationType, InitialDisbursalOnActivation,
    InterestPostingAccountIds, InterestReceivable, InterestReceivableAccountSets,
    InterestReceivableAccountType, InternalAccountSetDetails, LedgerOmnibusAccountIds, LedgerTxId,
    LongTermDisbursedIntegrationMeta, LongTermInterestIntegrationMeta,
    OverdueDisbursedIntegrationMeta, PendingCreditFacility, PendingCreditFacilityAccountIds,
    PendingCreditFacilityBalanceSummary, PendingCreditFacilityCreation, PendingCreditFacilityId,
    ResolvedChartOfAccountsIntegrationConfig, Satoshis, ShortTermDisbursedIntegrationMeta,
    ShortTermInterestIntegrationMeta, StructuringFeeOnActivation, UsdCents,
};

use core_credit_collection::Obligation;

use crate::{constants::*, credit_templates as templates, velocity};
...
impl CreditLedger {
    ...
    pub async fn init(
        ...
        let mut summary_ids: HashMap<&str, InternalAccountSetDetails> = HashMap::new();
        for spec in catalog.summary_specs() {
        ...
        let get = |spec: &CreditSummaryAccountSetSpec| -> InternalAccountSetDetails {
            summary_ids[spec.external_ref]
        };

        let disbursed_receivable = DisbursedReceivable {
            short_term: DisbursedReceivableAccountSets {
                individual: get(&summary.short_term_individual_disbursed_receivable),
                government_entity: get(&summary.short_term_government_entity_disbursed_receivable),
                private_company: get(&summary.short_term_private_company_disbursed_receivable),
                bank: get(&summary.short_term_bank_disbursed_receivable),
                financial_institution: get(
                    &summary.short_term_financial_institution_disbursed_receivable
                ),
                foreign_agency_or_subsidiary: get(
                    &summary.short_term_foreign_agency_or_subsidiary_disbursed_receivable
                ),
                non_domiciled_company: get(
                    &summary.short_term_non_domiciled_company_disbursed_receivable
                ),
            },
            long_term: DisbursedReceivableAccountSets {
                individual: get(&summary.long_term_individual_disbursed_receivable),
                government_entity: get(&summary.long_term_government_entity_disbursed_receivable),
                private_company: get(&summary.long_term_private_company_disbursed_receivable),
                bank: get(&summary.long_term_bank_disbursed_receivable),
                financial_institution: get(
            ...
            overdue: DisbursedReceivableAccountSets {
                individual: get(&summary.overdue_individual_disbursed_receivable),
                government_entity: get(&summary.overdue_government_entity_disbursed_receivable),
                private_company: get(&summary.overdue_private_company_disbursed_receivable),
                bank: get(&summary.overdue_bank_disbursed_receivable),
                financial_institution: get(
        ...
        let interest_receivable = InterestReceivable {
            short_term: InterestReceivableAccountSets {
                individual: get(&summary.short_term_individual_interest_receivable),
                government_entity: get(&summary.short_term_government_entity_interest_receivable),
                private_company: get(&summary.short_term_private_company_interest_receivable),
                bank: get(&summary.short_term_bank_interest_receivable),
                financial_institution: get(
            ...
            long_term: InterestReceivableAccountSets {
                individual: get(&summary.long_term_individual_interest_receivable),
                government_entity: get(&summary.long_term_government_entity_interest_receivable),
                private_company: get(&summary.long_term_private_company_interest_receivable),
                bank: get(&summary.long_term_bank_interest_receivable),
                financial_institution: get(
        ...
        let internal_account_sets = CreditFacilityInternalAccountSets {
            ...
            disbursed_defaulted: get(&summary.credit_disbursed_defaulted),
            interest_receivable,
            interest_defaulted: get(&summary.credit_interest_defaulted),
            interest_income: get(&summary.credit_interest_income),
            fee_income: get(&summary.credit_fee_income),
            uncovered_outstanding: get(&summary.credit_uncovered_outstanding),
            payment_holding: get(&summary.credit_payment_holding),
        };
            ...
            .await
        {
            ...
            Err(e) => return Err(CreditLedgerError::from_ledger(e)),
        }
        ...
        Ok(Self {
            ...
            clock,
            journal_id,
            facility_omnibus_account_ids: omnibus_ids
                [omnibus.credit_facility_omnibus.account_set_ref],
            collateral_omnibus_account_ids: omnibus_ids
                [omnibus.credit_collateral_omnibus.account_set_ref],
            liquidation_proceeds_omnibus_account_ids: omnibus_ids[omnibus
                .credit_facility_liquidation_proceeds_omnibus
                .account_set_ref],
            interest_added_to_obligations_omnibus_account_ids: omnibus_ids[omnibus
                .credit_interest_added_to_obligations_omnibus
                .account_set_ref],
            payments_made_omnibus_account_ids: omnibus_ids
        ...
        match cala.account_sets().create(new_account_set).await {
            ...
            Err(cala_ledger::account_set::error::AccountSetError::ExternalIdAlreadyExists) => {
                Ok(cala
                    ...
                    .await
                    .map_err(CreditLedgerError::from_ledger)?
                    .id)
            }
    ...
    async fn find_or_create_omnibus_account(
            ...
            .await
        {
            Ok(account) => {
                ...
                op.commit().await.map_err(CreditLedgerError::from_ledger)?;
    ...
    pub async fn get_collateral_for_pending_facility(
        &self,
        collateral_account_id: CalaAccountId,
    ...
    pub async fn handle_pending_facility_creation_in_op(
        &self,
        op: &mut es_entity::DbOp<'_>,
...
impl CreditLedgerOps for CreditLedger {
    async fn get_pending_credit_facility_balance(
        &self,
        account_ids: PendingCreditFacilityAccountIds,
        collateral_account_id: CalaAccountId,
    ) -> Result<PendingCreditFacilityBalanceSummary, CreditLedgerError> {
        self.get_pending_credit_facility_balance(account_ids, collateral_account_id)
            .await
    }

    async fn get_collateral_for_pending_facility(
        &self,
        collateral_account_id: CalaAccountId,
    ) -> Result<Satoshis, CreditLedgerError> {
        self.get_collateral_for_pending_facility(collateral_account_id)
            .await
    }

    async fn get_credit_facility_balance(
        &self,
        account_ids: CreditFacilityLedgerAccountIds,
        collateral_account_id: CalaAccountId,
    ) -> Result<CreditFacilityBalanceSummary, CreditLedgerError> {
        self.get_credit_facility_balance(account_ids, collateral_account_id)
            .await
    }

    async fn handle_pending_facility_creation_in_op(
        &self,
        op: &mut es_entity::DbOp<'_>,
        pending_credit_facility: &PendingCreditFacility,
        initiated_by: &(impl SystemSubject + Send + Sync),
    ) -> Result<(), CreditLedgerError> {
        self.handle_pending_facility_creation_in_op(op, pending_credit_facility, initiated_by)
            .await
    }

    async fn handle_activation_in_op(
        &self,
        op: &mut es_entity::DbOpWithTime<'_>,
        activation: CreditFacilityActivation,
        initiated_by: &(impl SystemSubject + Send + Sync),
    ) -> Result<(), CreditLedgerError> {
        self.handle_activation_in_op(op, activation, initiated_by)
            .await
    }

    async fn record_interest_accrual_in_op(
        &self,
        op: &mut es_entity::DbOp<'_>,
        accrual: CreditFacilityInterestAccrual,
        initiated_by: &(impl SystemSubject + Send + Sync),
    ) -> Result<(), CreditLedgerError> {
        self.record_interest_accrual_in_op(op, accrual, initiated_by)
            .await
    }

    async fn record_interest_accrual_cycle_in_op(
        &self,
        op: &mut es_entity::DbOp<'_>,
        cycle: CreditFacilityInterestAccrualCycle,
        initiated_by: &(impl SystemSubject + Send + Sync),
    ) -> Result<(), CreditLedgerError> {
        self.record_interest_accrual_cycle_in_op(op, cycle, initiated_by)
            .await
    }

    async fn initiate_disbursal_in_op(
        &self,
        op: &mut es_entity::DbOp<'_>,
        entity_id: DisbursalId,
        tx_id: LedgerTxId,
        ...
        self.initiate_disbursal_in_op(op, entity_id, tx_id, amount, account_ids, initiated_by)
            .await
    }

    async fn cancel_disbursal_in_op(
        &self,
        op: &mut es_entity::DbOpWithTime<'_>,
        entity_id: DisbursalId,
        tx_id: LedgerTxId,
        ...
        self.cancel_disbursal_in_op(op, entity_id, tx_id, amount, account_ids, initiated_by)
            .await
    }

    async fn settle_disbursal_in_op(
        &self,
        op: &mut es_entity::DbOpWithTime<'_>,
        entity_id: DisbursalId,
        disbursed_into_account_id: CalaAccountId,
    ...
    async fn attach_chart_of_accounts_account_sets_in_op(
        &self,
        op: &mut es_entity::DbOpWithTime<'_>,
        new_integration_config: &ResolvedChartOfAccountsIntegrationConfig,
        old_integration_config: Option<&ResolvedChartOfAccountsIntegrationConfig>,
    ) -> Result<(), CreditLedgerError> {
        self.attach_chart_of_accounts_account_sets_in_op(
            op,
            new_integration_config,
            old_integration_config,
        )
    ...
    async fn attach_short_term_disbursed_receivable_account_sets_in_op(
        &self,
        op: &mut es_entity::DbOpWithTime<'_>,
        new_integration_meta: &ShortTermDisbursedIntegrationMeta,
        old_integration_meta: Option<&ShortTermDisbursedIntegrationMeta>,
    ...
    async fn attach_long_term_disbursed_receivable_account_sets_in_op(
        &self,
        op: &mut es_entity::DbOpWithTime<'_>,
        new_integration_meta: &LongTermDisbursedIntegrationMeta,
        old_integration_meta: Option<&LongTermDisbursedIntegrationMeta>,
