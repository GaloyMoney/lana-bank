...
BEGIN
  ...
  IF event_type NOT IN ('initialized', 'interest_accrual_cycle_started', 'interest_accrual_cycle_concluded', 'collateralization_state_changed', 'collateralization_ratio_changed', 'partial_liquidation_initiated', 'proceeds_from_partial_liquidation_applied', 'collateralization_thresholds_updated', 'matured', 'completed', 'activated') THEN
...
;
    ...
    new_row.lower_price_threshold := (NEW.event ->> 'lower_price_threshold')::BIGINT;
...
;
    ...
    new_row.upper_price_threshold := (NEW.event ->> 'upper_price_threshold')::BIGINT;
  ELSE
    ...
    new_row.lower_price_threshold := current_row.lower_price_threshold;
    ...
    new_row.upper_price_threshold := current_row.upper_price_threshold;
  END IF;
  ...
  CASE event_type
    ...
    WHEN 'proceeds_from_partial_liquidation_applied' THEN
      new_row.liquidation_id := (NEW.event ->> 'liquidation_id')::UUID;
    WHEN 'collateralization_thresholds_updated' THEN
      new_row.lower_price_threshold := (NEW.event ->> 'lower_price_threshold')::BIGINT;
      new_row.upper_price_threshold := (NEW.event ->> 'upper_price_threshold')::BIGINT;
    WHEN 'matured' THEN
