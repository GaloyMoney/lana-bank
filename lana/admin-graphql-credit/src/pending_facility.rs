...
impl PendingCreditFacilityBase {
    ...
    async fn wallet(
        &self,
        ctx: &Context<'_>,
    ) -> async_graphql::Result<Option<admin_graphql_custody::WalletBase>> {
        let (app, _sub) = app_and_sub_from_ctx!(ctx);
        let collaterals: std::collections::HashMap<_, crate::collateral::DomainCollateral> = app
            .credit()
            .collaterals()
        ...
        if let Some(wallet_id) = collateral.custody_wallet_id {
            let wallets: std::collections::HashMap<_, admin_graphql_custody::WalletBase> =
