...
use crate::{deposit_account_history::*, ledger_accounts::*};
...
impl DepositAccountBase {
    ...
    async fn deposits(
        &self,
        ctx: &Context<'_>,
    ) -> async_graphql::Result<Vec<crate::deposit::DepositBase>> {
        let (app, sub) = app_and_sub_from_ctx!(ctx);
        let deposits = app
            .deposits()
        ...
        Ok(deposits
            .into_iter()
            .map(crate::deposit::DepositBase::from)
            .collect())
    }
    ...
    async fn withdrawals(
        &self,
        ctx: &Context<'_>,
    ) -> async_graphql::Result<Vec<crate::withdrawal::WithdrawalBase>> {
        let (app, sub) = app_and_sub_from_ctx!(ctx);
        let withdrawals = app
            .deposits()
        ...
        Ok(withdrawals
            .into_iter()
            .map(crate::withdrawal::WithdrawalBase::from)
            .collect())
    }
