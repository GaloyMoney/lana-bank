...
use core_credit_collection::{
    CalaAccountId, CollectionLedgerError, CollectionLedgerOps, ObligationDefaultedReallocationData,
    ObligationDueReallocationData, ObligationOverdueReallocationData, Payment, PaymentAllocation,
};
...
impl CollectionLedger {
    ...
    pub async fn record_payment_in_op(
            ...
            .await
        {
            ...
            Err(e) => return Err(CollectionLedgerError::from_ledger(e)),
            _ => (),
        };
    ...
    pub async fn record_obligation_due_in_op(
        ...
        self.cala
            ...
            .await
            .map_err(CollectionLedgerError::from_ledger)?;
        Ok(())
    }
...
impl CollectionLedgerOps for CollectionLedger {
    async fn record_payment_in_op(
        ...
        self.record_payment_in_op(op, payment, initiated_by).await
    }

    async fn record_payment_allocations_in_op(
        &self,
        op: &mut es_entity::DbOp<'_>,
        payments: Vec<PaymentAllocation>,
        initiated_by: &(impl SystemSubject + Send + Sync),
    ) -> Result<(), CollectionLedgerError> {
        self.record_payment_allocations_in_op(op, payments, initiated_by)
            .await
    }

    async fn record_obligation_due_in_op(
        &self,
        op: &mut es_entity::DbOp<'_>,
        data: ObligationDueReallocationData,
        initiated_by: &(impl SystemSubject + Send + Sync),
    ) -> Result<(), CollectionLedgerError> {
        self.record_obligation_due_in_op(op, data, initiated_by)
            .await
    }

    async fn record_obligation_overdue_in_op(
        &self,
        op: &mut es_entity::DbOp<'_>,
        data: ObligationOverdueReallocationData,
        initiated_by: &(impl SystemSubject + Send + Sync),
    ) -> Result<(), CollectionLedgerError> {
        self.record_obligation_overdue_in_op(op, data, initiated_by)
            .await
    }

    async fn record_obligation_defaulted_in_op(
        &self,
        op: &mut es_entity::DbOp<'_>,
        data: ObligationDefaultedReallocationData,
        initiated_by: &(impl SystemSubject + Send + Sync),
    ) -> Result<(), CollectionLedgerError> {
        self.record_obligation_defaulted_in_op(op, data, initiated_by)
            .await
    }
}
