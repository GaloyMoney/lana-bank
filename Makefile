...
e2e: clean-deps start-deps
	SQLX_OFFLINE=true cargo build --bin lana-cli --bin lana-admin-cli
	bats -t bats
...
docs-autogenerate: update-public-event-schemas
	cd docs-site && pnpm install && pnpm run generate-api-docs && pnpm run generate-events-docs

# Generate docs and add TODO placeholders for new operations
prep-docs:
	cd docs-site && \
		pnpm run generate-api-docs && \
		pnpm run generate-versioned-api-docs
