...
const CLI_AFTER_HELP: &str = r#"Examples:
  ...
  lana-admin workflow list --yaml > workflow.yml

  lana-admin workflow deps \
    --step credit_facility_disbursal_initiate
...
fn parse_uuid_arg(value: &str) -> Result<String, String> {
    uuid::Uuid::parse_str(value)
        .map(|uuid| uuid.to_string())
        .map_err(|_| "expected UUID".to_string())
}
...
pub enum Command {
    ...
    /// Inspect schema-derived workflow dependency graphs
    Workflow {
        #[command(subcommand)]
...
pub enum WorkflowAction {
    /// List all steps in the schema-derived dependency graph
    List {
        /// Output the full workflow graph as YAML instead of a table
        #[arg(long)]
        yaml: bool,
    },
    /// Show required steps for a target step in the schema-derived dependency graph
    Deps {
