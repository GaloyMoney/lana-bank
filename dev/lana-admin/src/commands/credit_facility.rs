        ...
        CreditFacilityAction::DisbursalInitiate {
            ...
            if json {
                ...
                if let Value::Object(ref mut obj) = value
                    && let Some(id) = obj.get("creditFacilityDisbursalId").cloned()
                {
                    // Keep backward compatibility for older automation/test scripts.
                    obj.insert("disbursalId".to_string(), id);
                }
        ...
        CreditFacilityAction::Find { id } => {
            ...
            match data.credit_facility {
                Some(f) => {
                    ...
                    if let Value::Object(ref mut obj) = value
                        && let Some(Value::Array(disbursals)) = obj.get_mut("disbursals")
                    {
                        for disbursal in disbursals {
                            if let Value::Object(disbursal_obj) = disbursal
                                && let Some(id) =
                                    disbursal_obj.get("creditFacilityDisbursalId").cloned()
                            {
                                // Keep backward compatibility for older automation/test scripts.
                                disbursal_obj.insert("disbursalId".to_string(), id);
                            }
                        }
                    }
                    output::print_json(&value)?;
