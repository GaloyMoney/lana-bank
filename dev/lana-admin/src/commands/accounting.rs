    ...
    match action {
        ...
        AccountingAction::BaseConfig => {
            ...
            match data.chart_of_accounts.accounting_base_config {
                ...
                None => output::not_found("Accounting base config", json),
            }
        }
        AccountingAction::CreditConfig => {
            ...
            match data.credit_config {
                ...
                None => output::not_found("Credit config", json),
            }
        }
        AccountingAction::DepositConfig => {
            ...
            match data.deposit_config {
                ...
                None => output::not_found("Deposit config", json),
            }
        }
        ...
        AccountingAction::LedgerAccount { code } => {
            ...
            match data.ledger_account_by_code {
                ...
                None => output::not_found("Ledger account", json),
            }
        }
        AccountingAction::FiscalYear { action } => {
            crate::commands::fiscal_year::execute(client, action, json).await?;
        }
        AccountingAction::Statement { action } => {
            crate::commands::financial_statement::execute(client, action, json).await?;
        }
        AccountingAction::Export { action } => {
            crate::commands::csv_export::execute(client, action, json).await?;
        }
    }
    Ok(())
}
