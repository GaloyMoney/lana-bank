                    ...
                    KeyCode::Enter => {
                        ...
                        if let Some(pending) = app.pending_action.take() {
                            ...
                            tokio::spawn(async move {
                                let result =
                                    data::execute_action(&cl, domain, &eid, action, Some(reason))
                                        .await;
                ...
                Screen::ListView => match key.code {
                    ...
                    KeyCode::Char('n') => {
                            ...
                            && app.list.has_next_page
                        {
                            ...
                            let tx = tx.clone();
                            tokio::spawn(async move {
                                let result = data::fetch_list(&cl, domain, 25, cursor).await;
                                let _ = tx.send(AsyncResult::ListLoaded(domain, result));
                            });
                        }
                    }
                    ...
                    KeyCode::Char(c) => {
                            ...
                            app.detail.actions.iter().find(|a| a.key() == c).copied()
                        {
                            if action.needs_input() {
                                ...
                                tokio::spawn(async move {
                                    let result =
                                        data::execute_action(&cl, domain, &eid, action, None).await;
                                    let _ = tx.send(AsyncResult::ActionDone(result));
                                });
                            }
                        }
