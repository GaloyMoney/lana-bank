...
(
    select order_by, title, eng_title, to_json_string(source_account_spaced_codes) as source_account_spaced_codes_json
    from {{ ref('static_ncf_01_02_account_config_legacy') }}
    except distinct
    select order_by, title, eng_title, to_json_string(source_account_spaced_codes) as source_account_spaced_codes_json
