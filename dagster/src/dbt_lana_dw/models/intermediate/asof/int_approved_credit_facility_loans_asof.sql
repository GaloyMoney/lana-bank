...
with
    approved_credit_facilities as (
        select *
        from {{ ref("int_core_credit_facility_events_rollup_asof") }}
        where approved
    ),
    ...
    collateral_deposits as (
        ...
        where credit_facility_modified_at <= {{ as_of_timestamp() }}
    ...
    disbursals as (
        ...
        from {{ ref("int_core_disbursal_events_rollup_asof") }}
        where is_settled
    ),
    ...
    interest as (
        ...
        from {{ ref("int_core_interest_accrual_cycle_events_rollup_asof") }}
        group by credit_facility_id
    ),
    ...
    payments as (
        ...
        from {{ ref("int_payment_events_asof") }}
        group by credit_facility_id
    ),
    ...
    final as (
        select
            ...
            credit_facility_maturity_at < {{ as_of_date() }} as matured
