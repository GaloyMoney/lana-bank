...
select
    ...
    defaulted_account_id,
    defaulted_amount,
    cast(json_value(defaulted_date) as timestamp) as defaulted_date,
    due_amount,
    cast(due_date as timestamp) as due_date,
    effective,
    cast(json_value(liquidation_date) as timestamp) as liquidation_date,
    payment_allocation_amount,
    obligation_type,
    overdue_amount,
    cast(json_value(overdue_date) as timestamp) as overdue_date,
    payment_id,
    reference,
    ledger_tx_ids,
    payment_allocation_ids,
