...
export const mockJournalEntriesSort = (overrides?: Partial<JournalEntriesSort>, _relationshipsToOmit: Set<string> = new Set()): JournalEntriesSort => {
    const relationshipsToOmit: Set<string> = new Set(_relationshipsToOmit);
    relationshipsToOmit.add('JournalEntriesSort');
    return {
        by: overrides && overrides.hasOwnProperty('by') ? overrides.by! : JournalEntriesSortBy.CreatedAt,
        direction: overrides && overrides.hasOwnProperty('direction') ? overrides.direction! : SortDirection.Asc,
    };
};
