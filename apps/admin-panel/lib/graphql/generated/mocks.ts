...
export const mockCreditFacilityExport = (overrides?: Partial<CreditFacilityExport>, _relationshipsToOmit: Set<string> = new Set()): { __typename: 'CreditFacilityExport' } & CreditFacilityExport => {
    ...
    return {
        ...
        createdAt: overrides && overrides.hasOwnProperty('createdAt') ? overrides.createdAt! : generateMockValue.timestamp(),
        id: overrides && overrides.hasOwnProperty('id') ? overrides.id! : faker.string.uuid(),
        status: overrides && overrides.hasOwnProperty('status') ? overrides.status! : PdfGenerationStatus.Completed,
    };
};
