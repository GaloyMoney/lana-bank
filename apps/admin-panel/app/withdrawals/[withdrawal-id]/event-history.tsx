...
export const WithdrawalEventHistory: React.FC<WithdrawalEventHistoryProps> = ({
  ...
  const t = useTranslations("Withdrawals.WithdrawDetails.eventHistory")
  const te = useTranslations("EntityEvents.withdrawal")

  const { data, loading } = useWithdrawalEventHistoryQuery({
    variables: { id: withdrawalPublicId, first: 100 },
  })

  const events = data?.withdrawalByPublicId?.eventHistory.nodes ?? []
