...
- For a target step, run `workflow deps --step <STEP>` first, then execute the returned steps in order.
...
- `workflow deps` includes read-only bridge steps when they are part of the real dependency path, including query steps such as `pending_credit_facility`.
