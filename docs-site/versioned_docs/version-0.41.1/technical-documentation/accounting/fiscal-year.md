...
- On initial startup if the accounting initialization configuration has an opening date set.
- Via the admin panel or system API.

## Closing Months of a FiscalYear

Monthly closes lock the entire ledger against transactions with an `effective_date` before `month_closed_as_of`. To execute this entity command, the precondition that the month has past (according to the current system time) must be satisfied. The command applies to the oldest, unclosed month of the `FiscalYear`.
