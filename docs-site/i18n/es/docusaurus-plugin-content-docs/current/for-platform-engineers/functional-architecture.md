...
Los servicios externos notifican al sistema a través de webhooks. **SumSub** envía notificaciones sobre el ciclo de vida de la verificación KYC a `/webhook/sumsub`. Cuando un cliente completa su verificación, SumSub notifica el resultado (aprobado o rechazado). El sistema procesa esta notificación y actualiza el estado KYC del cliente, lo que puede habilitar la creación de cuentas de depósito o facilidades de crédito según la configuración.

Los **custodios de Bitcoin** que admiten webhooks (BitGo, Komainu) reportan eventos de billetera a `/webhook/custodian/[provider]`. Cada proveedor tiene su propio formato de webhook que el sistema normaliza. Los eventos típicos incluyen depósitos de Bitcoin en billeteras de colateral. Al recibir una notificación, el sistema verifica su autenticidad (normalmente mediante HMAC), identifica la billetera afectada, actualiza el saldo de colateral correspondiente y recalcula el CVL de la facilidad de crédito asociada. Si el nuevo CVL cruza un umbral configurado, el estado de colateralización se actualiza y se publican los eventos correspondientes.

La integración con **Bitfinex** es diferente. Lana utiliza el endpoint autenticado de dirección de depósito de Bitfinex para crear una nueva dirección de depósito de Bitcoin para una facilidad específica desde un tipo de billetera configurado en Bitfinex (`exchange`, `margin` o `funding`). Esto brinda al banco otra opción de custodia para la generación de direcciones, aunque actualmente no hay sincronización de colateral mediante webhook, por lo que las actualizaciones de saldo deben realizarse manualmente o mediante un flujo de trabajo de sondeo futuro.

Este flujo de webhooks es crítico para la gestión de riesgos en tiempo real. Sin él, el sistema dependería de sondeos periódicos, lo que podría retrasar la visibilidad de los cambios en el colateral y aumentar el riesgo durante caídas en el precio de Bitcoin.
