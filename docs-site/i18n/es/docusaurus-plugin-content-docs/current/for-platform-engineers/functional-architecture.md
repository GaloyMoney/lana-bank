...
El proceso de verificación KYC se integra con SumSub. Un cliente comienza en estado **PendingVerification**. Cuando SumSub notifica mediante webhook que la verificación fue exitosa, el cliente pasa a **Verified** con un nivel KYC (Básico o Avanzado). Si la verificación falla, permanece en estado **Rejected**. El sistema puede configurarse para exigir la verificación antes de permitir la creación de cuentas de depósito o líneas de crédito.
...
Para cumplir con la normativa de cuentas inactivas, el sistema registra la última actividad de cada cuenta de depósito. Un proceso periódico clasifica automáticamente las cuentas según su actividad: **Active** si han tenido actividad reciente (menos de un año), **Inactive** si han estado entre uno y diez años sin actividad, y **Escheatable** si superan los diez años. Esta clasificación es independiente del estado operativo de la cuenta y, por sí sola, no congela ni cierra la cuenta.

#### 2.1.4 Custodia

El módulo de custodia ofrece una abstracción sobre varios proveedores de custodia de Bitcoin, permitiendo al banco trabajar con distintos custodios según sus necesidades operativas y regulatorias. El sistema está diseñado con un patrón de plugins donde cada **Custodian** implementa una interfaz común. Actualmente están implementados **BitGo**, **Komainu** y **Bitfinex**, y la arquitectura permite añadir nuevos custodios sin modificar el resto del sistema.

En cada despliegue, pueden configurarse y activarse varios custodios simultáneamente. Al crear una línea de crédito, se puede especificar qué custodio gestionará la garantía de esa línea en particular. Esto permite, por ejemplo, utilizar diferentes custodios para distintos segmentos de clientes o jurisdicciones.

Cada custodio administra **Wallets** que se asignan a líneas de crédito para recibir la garantía en Bitcoin. Los custodios notifican al sistema sobre cambios en los saldos de las billeteras mediante webhooks. Cuando llega una notificación, el sistema actualiza la **Collateral** asociada con la línea correspondiente y recalcula el CVL. Esta sincronización automática es fundamental para mantener una visión precisa del riesgo en tiempo real.

Los webhooks de los custodios se reciben en endpoints específicos de cada proveedor y se validan criptográficamente antes de ser procesados. La configuración de cada custodio incluye las credenciales y llaves API necesarias para verificar la autenticidad del webhook. Las llaves sensibles se almacenan cifradas.
...
Los servicios externos notifican al sistema a través de webhooks. **SumSub** envía notificaciones sobre el ciclo de vida de la verificación KYC a `/webhook/sumsub`. Cuando un cliente completa su verificación, SumSub notifica el resultado (aprobado o rechazado). El sistema procesa esta notificación y actualiza el estado KYC del cliente, lo que puede permitir la creación de cuentas de depósito o líneas de crédito según la configuración.

Los **custodios de Bitcoin** que admiten webhooks (BitGo, Komainu) notifican eventos de billetera a `/webhook/custodian/[provider]`. Cada proveedor tiene su propio formato de webhook que el sistema normaliza. Los eventos típicos incluyen depósitos de Bitcoin en las billeteras de garantía. Cuando llega una notificación, el sistema verifica su autenticidad (normalmente mediante HMAC), identifica la billetera afectada, actualiza el saldo de la garantía correspondiente y recalcula el CVL de la línea de crédito asociada. Si el nuevo CVL supera algún umbral configurado, se actualiza el estado de colateralización y se publican los eventos correspondientes.

**Bitfinex** está integrado de manera diferente. Lana utiliza el endpoint autenticado de direcciones de depósito de Bitfinex para crear una nueva dirección de depósito de Bitcoin para una línea de crédito desde una billetera de Bitfinex configurada. Esto da al banco otra opción de custodia para la generación de direcciones, pero actualmente no proporciona sincronización de garantía basada en webhooks, por lo que la actualización de saldos debe hacerse mediante operaciones manuales o un futuro flujo de trabajo por sondeo.

Este flujo de webhooks es fundamental para la gestión de riesgos en tiempo real. Sin él, el sistema dependería de sondeos periódicos y podría tener una visibilidad retrasada de los cambios en las garantías, aumentando el riesgo durante caídas del precio de Bitcoin.
