...
El módulo de custodia proporciona una abstracción sobre varios proveedores de custodia de Bitcoin, permitiendo que el banco trabaje con diferentes custodios según sus necesidades operativas y regulatorias. El sistema está diseñado con un patrón de complemento donde cada **Custodio** implementa una interfaz común. Se han implementado opciones como **BitGo**, **Komainu** y una opción de **Autocustodia** basada en xpub, y la arquitectura permite añadir nuevos custodios sin modificar el resto del sistema.

En cada implementación, se pueden configurar y activar varios custodios simultáneamente. Cuando se crea una línea de crédito, puedes especificar qué custodio gestionará la garantía de esa instalación en particular. Esto permite, por ejemplo, usar diferentes custodios para distintos segmentos de clientes o jurisdicciones.

Cada custodio gestiona **Carteras** que se asignan a las líneas de crédito para recibir garantías en Bitcoin. Los custodios alojados notifican al sistema sobre cambios en los saldos de las carteras a través de webhooks. La opción de autocustodia almacena solo un `xpub` de la cuenta en el backend, deriva una nueva dirección de recepción para cada préstamo creado y monitoriza los cambios de saldo confirmados consultando un backend de esplora. En ambos casos, Lana actualiza la **Garantía** asociada con la instalación correspondiente y recalcula automáticamente el CVL.

Los webhooks de los custodios se reciben en puntos finales específicos del proveedor y se validan criptográficamente antes de procesarse. La configuración para custodios alojados incluye las credenciales de API necesarias y claves para verificar la autenticidad de los webhooks, y esos valores sensibles se almacenan cifrados. Para autocustodia, el backend mantiene solo el `xpub`; el `xpriv` correspondiente se genera y conserva fuera del backend.
