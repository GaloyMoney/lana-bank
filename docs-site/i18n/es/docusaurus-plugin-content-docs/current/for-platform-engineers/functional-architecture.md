...
El módulo de custodia proporciona una abstracción sobre múltiples proveedores de custodia de Bitcoin, permitiendo al banco trabajar con distintos custodios según sus necesidades operativas y regulatorias. El sistema está diseñado con un patrón de plugins, donde cada **Custodian** implementa una interfaz común. Están implementadas las opciones de **BitGo**, **Komainu** y una **Auto-Custodia** basada en xpub, y la arquitectura permite añadir nuevos custodios sin modificar el resto del sistema.

En cada implementación, se pueden configurar y activar varios custodios simultáneamente. Al crear una línea de crédito, se puede especificar qué custodio gestionará la garantía de esa línea en particular. Esto permite, por ejemplo, usar distintos custodios para diferentes segmentos de clientes o jurisdicciones.

Cada custodio gestiona **Wallets** que se asignan a las líneas de crédito para recibir la garantía en Bitcoin. Los custodios alojados notifican al sistema sobre cambios en los saldos de las billeteras mediante webhooks. La opción de auto-custodia almacena solo una `xpub` de cuenta en el backend, deriva una nueva dirección de recepción para cada préstamo creado y consulta a un backend esplora para detectar cambios confirmados en el saldo. La URL base de esplora se selecciona al iniciar el sistema desde `lana.yml` en función de la red de auto-custodia configurada. El flujo de auto-custodia es compatible con configuraciones de cuentas mainnet, testnet3, testnet4 y signet. En ambos casos, Lana actualiza automáticamente la **Garantía** asociada a la línea correspondiente y recalcula el CVL.

Los webhooks de los custodios se reciben en endpoints específicos de cada proveedor y se validan criptográficamente antes de su procesamiento. La configuración para custodios alojados incluye las credenciales API y claves necesarias para verificar la autenticidad de los webhooks, y esos valores sensibles se almacenan cifrados. Para la auto-custodia, el backend mantiene solo la `xpub`; la `xpriv` correspondiente se genera y conserva fuera del backend, mientras que las URLs esplora específicas de la red se suministran a través de la configuración de inicio.
