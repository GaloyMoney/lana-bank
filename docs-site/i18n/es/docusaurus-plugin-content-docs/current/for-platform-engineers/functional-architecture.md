...
El módulo de custodia proporciona una abstracción sobre varios proveedores de custodia de Bitcoin, permitiendo al banco trabajar con diferentes custodios según sus necesidades operativas y regulatorias. El sistema está diseñado con un patrón de plugin donde cada **Custodian** implementa una interfaz común. Se han implementado opciones como **BitGo**, **Komainu** y una opción de **Autocustodia** basada en xpub, y la arquitectura permite añadir nuevos custodios sin modificar el resto del sistema.

En cada implementación, se pueden configurar y activar múltiples custodios simultáneamente. Al crear una línea de crédito, puedes especificar qué custodio gestionará la garantía de esa línea en particular. Esto permite, por ejemplo, asignar distintos custodios para diferentes segmentos de clientes o jurisdicciones.

Cada custodio gestiona **Wallets** que se asignan a líneas de crédito para recibir garantía en Bitcoin. Los custodios alojados informan al sistema sobre cambios en los saldos de las wallets mediante webhooks. La opción de autocustodia solo almacena una `xpub` de la cuenta en el backend, deriva una nueva dirección de recepción para cada préstamo creado y consulta a un backend de esplora para los cambios confirmados de saldo. El flujo de autocustodia es compatible con configuraciones de cuenta de mainnet, testnet3, testnet4 y signet. En ambos casos, Lana actualiza la **Collateral** asociada a la línea correspondiente y recalcula automáticamente el CVL.

Los webhooks del custodio se reciben en endpoints específicos de cada proveedor y se validan criptográficamente antes de su procesamiento. La configuración para custodios alojados incluye las credenciales de API y claves necesarias para verificar la autenticidad de los webhooks, y dichos valores sensibles se almacenan encriptados. En el caso de autocustodia, el backend solo conserva la `xpub`; la `xpriv` correspondiente se genera y se guarda fuera del backend.
