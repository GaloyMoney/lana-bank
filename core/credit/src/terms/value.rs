...
use crate::{
   ...
   primitives::{
       CVLPct, CollateralizationState, DisbursalType, DisbursedReceivableAccountCategory, PriceOfOneBTC,
       Satoshis, UsdCents,
   },
};
...
#[cfg_attr(feature = "json-schema", derive(JsonSchema))]
#[serde(tag = "type", content = "value", rename_all = "snake_case")]
pub enum FacilityDisbursalRule {
   OnCreditFacilityActivation,
   Revolver,
}


impl Default for FacilityDisbursalRule {
   fn default() -> Self {
       Self::Revolver
   }
}
...
impl From<FacilityDisbursalRule> for DisbursalType {
   fn from(disbursal_rule: FacilityDisbursalRule) -> Self {
       match disbursal_rule {
           FacilityDisbursalRule::OnCreditFacilityActivation => DisbursalType::OnCreditFacilityActivation,
           FacilityDisbursalRule::Revolver => DisbursalType::Revolver,
       }
   }
}
...
pub struct TermValues {
   ...
   #[builder(setter(into))]
   pub facility_disbursal_rule: Option<FacilityDisbursalRule>,
}
...
impl TermValues {
   ...
   pub fn disbursal_rule(&self) -> FacilityDisbursalRule {
       self.facility_disbursal_rule.unwrap_or_default()
   }
}
...
impl TermValuesBuilder {
   fn validate(&self) -> Result<(), TermsError> {
       ...
       let liquidation_cvl = self
        ...
        let structuring_fee_rate = self
            .one_time_fee_rate
            .ok_or(UninitializedFieldError::new("one_time_fee_rate"))?;
        let disbursal_rule = self
            .facility_disbursal_rule
            .flatten()
            .unwrap_or_default();
       ...
       if !structuring_fee_rate.0.is_zero() {
        if disbursal_rule != FacilityDisbursalRule::Revolver || 
            disbursal_rule != FacilityDisbursalRule::OnCreditFacilityActivation {
            return Err(TermsError::UnAppliedStructuringFee);
        }
       }
