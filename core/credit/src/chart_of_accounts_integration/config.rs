    ...
    pub fn try_new(
        ...
        let off_balance_sheet_account_set_member_parent_id =
            |code: &AccountCode| -> Result<CalaAccountSetId, ChartOfAccountsIntegrationError> {
                Ok(chart.validated_account_set_id(code, AccountCategory::OffBalanceSheet)?)
            };

        let revenue_account_set_member_parent_id =
            |code: &AccountCode| -> Result<CalaAccountSetId, ChartOfAccountsIntegrationError> {
                Ok(chart.validated_account_set_id(code, AccountCategory::Revenue)?)
            };

        let asset_account_set_member_parent_id =
            |code: &AccountCode| -> Result<CalaAccountSetId, ChartOfAccountsIntegrationError> {
                Ok(chart.validated_account_set_id(code, AccountCategory::Asset)?)
            };
