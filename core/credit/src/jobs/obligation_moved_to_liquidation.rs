...
const CREDIT_FACILITY_MOVED_TO_LIQUIDATION_PROCESSING_JOB: JobType =
    JobType::new("credit-facility-moved-to-liquidation-processing");
    ...
    E: OutboxEventMarker<CoreCreditEvent>,
{
    ...
    where
        Self: Sized,
    {
        CREDIT_FACILITY_MOVED_TO_LIQUIDATION_PROCESSING_JOB
    }
    ...
    E: OutboxEventMarker<CoreCreditEvent>,
{
    async fn run(
        ...
        let moved_to_liquidation = if let Some(new_liquidation_obligation) = self
            .obligations
            .move_to_liquidation_in_op(&mut db, self.config.obligation_id, self.config.effective)
            .await?
        {
            new_liquidation_obligation
        } else {
            return Ok(JobCompletion::Complete);
        };

        self.ledger
            .move_to_liquidation_obligation(db, moved_to_liquidation)
            .await?;
