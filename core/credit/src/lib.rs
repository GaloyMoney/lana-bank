        ...
        + OutboxEventMarker<CoreCustomerEvent>,
{
    ...
    pub async fn complete_facility(
        ...
        let facility_for_lookup = self.facilities.find_by_id_without_audit(id).await?;
        let collateral = self
            .collaterals
            .find_by_id_without_audit(facility_for_lookup.collateral_id)
            .await?;
        let collateral_account_id = collateral.account_ids.collateral_account_id;
        ...
        let credit_facility = match self
            .facilities
            .complete_in_op(&mut db, id, CVLPct::UPGRADE_BUFFER, collateral_account_id)
            .await?
        {
