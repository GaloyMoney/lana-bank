// Omnibus Accounts
pub const CREDIT_COLLATERAL_OMNIBUS_ACCOUNT_SET_NAME: &str =
    "Credit Collateral Omnibus Account Set";
pub const CREDIT_COLLATERAL_OMNIBUS_ACCOUNT_SET_REF: &str = "credit-collateral-omnibus-account-set";
pub const CREDIT_COLLATERAL_OMNIBUS_ACCOUNT_REF: &str = "credit-collateral-omnibus-account";

pub const CREDIT_FACILITY_OMNIBUS_ACCOUNT_SET_NAME: &str = "Credit Facility Omnibus Account Set";
pub const CREDIT_FACILITY_OMNIBUS_ACCOUNT_SET_REF: &str = "credit-facility-omnibus-account-set";
pub const CREDIT_FACILITY_OMNIBUS_ACCOUNT_REF: &str = "credit-facility-omnibus-account";

// Summary Accounts
pub const CREDIT_FACILITY_REMAINING_ACCOUNT_SET_NAME: &str =
    "Credit Facility Remaining Account Set";
pub const CREDIT_FACILITY_REMAINING_ACCOUNT_SET_REF: &str = "credit-facility-remaining-account-set";

pub const CREDIT_COLLATERAL_ACCOUNT_SET_NAME: &str = "Credit Collateral Account Set";
pub const CREDIT_COLLATERAL_ACCOUNT_SET_REF: &str = "credit-collateral-account-set";

pub const CREDIT_DISBURSED_RECEIVABLE_REMAINING_ACCOUNT_SET_NAME: &str =
    "Credit Disbursed Receivable Account Set";
pub const CREDIT_DISBURSED_RECEIVABLE_REMAINING_ACCOUNT_SET_REF: &str =
    "credit-disbursed-receivable-account-set";

pub const CREDIT_INTEREST_RECEIVABLE_REMAINING_ACCOUNT_SET_NAME: &str =
    "Credit Interest Receivable Account Set";
pub const CREDIT_INTEREST_RECEIVABLE_REMAINING_ACCOUNT_SET_REF: &str =
    "credit-interest-receivable-account-set";

pub const CREDIT_INTEREST_INCOME_REMAINING_ACCOUNT_SET_NAME: &str =
    "Credit Interest Income Account Set";
pub const CREDIT_INTEREST_INCOME_REMAINING_ACCOUNT_SET_REF: &str =
    "credit-interest-income-account-set";

pub const CREDIT_FEE_INCOME_REMAINING_ACCOUNT_SET_NAME: &str = "Credit Fee Income Account Set";
pub const CREDIT_FEE_INCOME_REMAINING_ACCOUNT_SET_REF: &str = "credit-fee-income-account-set";

// Velocity Controls
pub(super) const CREDIT_FACILITY_VELOCITY_CONTROL_ID: uuid::Uuid =
    uuid::uuid!("00000000-0000-0000-0000-000000000002");
