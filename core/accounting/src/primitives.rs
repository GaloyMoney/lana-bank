...
// Re-export shared primitives from the lightweight crate
...
impl AccountSpec {
    pub fn try_new(
        ...
        let code = AccountCode::new(sections);
...
mod tests {
    ...
    fn errors_for_new_spec_if_invalid_parent() {
        ...
        let child_sections: Vec<AccountCodeSection> = child.into();
    ...
    mod is_parent_of {
        ...
        fn sections(code: AccountCode) -> Vec<AccountCodeSection> {
            code.into()
        }
        ...
        fn not_parent_when_child_sections_empty() {
            ...
            assert!(!parent.is_parent_of(&sections(child)));
        }
    ...
    mod accounting_base_config {
        ...
        fn try_new_ok_with_valid_config() {
            ...
            let gain_sections: Vec<AccountCodeSection> =
                (&config.equity_retained_earnings_gain_code).into();
            assert!(config.equity_code.is_parent_of(&gain_sections));
        }
