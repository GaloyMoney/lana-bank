...
pub use core_accounting_contracts::*;
...
mod tests {
    ...
    mod is_parent_of {
        ...
        fn not_parent_when_child_sections_empty() {
            ...
            assert!(!parent.is_parent_of(&child_sections));
        }
    ...
    mod accounting_base_config {
        ...
        fn try_new_ok_with_valid_config() {
            ...
            let retained_earnings_sections: Vec<AccountCodeSection> =
                config.equity_retained_earnings_gain_code.clone().into();
            assert!(config.equity_code.is_parent_of(&retained_earnings_sections));
        }
