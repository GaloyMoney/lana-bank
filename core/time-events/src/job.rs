...
const DAILY_CLOSING_BROADCASTER: JobType = JobType::new("time.daily-closing-broadcaster");
    ...
    E: OutboxEventMarker<CoreTimeEvent> + Send + Sync + 'static,
{
    ...
    #[instrument(name = "time.daily_closing_broadcaster_job.init", skip(self))]
    fn init(&self, _: &Job) -> Result<Box<dyn JobRunner>, Box<dyn std::error::Error>> {
        let broadcaster = DailyClosingBroadcaster::new(&self.outbox, self.config.clone());
...
where
    E: OutboxEventMarker<CoreTimeEvent> + Send + Sync + 'static,
{
    #[instrument(name = "time.daily_closing_broadcaster_job.run", skip_all, err)]
    async fn run(
        &self,
