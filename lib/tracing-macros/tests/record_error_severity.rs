...
async fn test_record_error_severity_with_boxstream() {
    ...
    impl StreamHolder {
        #[record_error_severity]
        async fn function_returning_boxstream_with_lifetime(&self) -> Result<BoxStream<'_, Arc<TestEvent>>, TestError>
        {
            // Simulate some async work
            tokio::time::sleep(std::time::Duration::from_millis(10)).await;
        ...
        #[record_error_severity]
        async fn function_returning_boxstream_with_lifetime_error(&self) -> Result<BoxStream<'_, Arc<TestEvent>>, TestError> {
            tokio::time::sleep(std::time::Duration::from_millis(10)).await;
            Err(TestError::Warning)
        }
    }
    ...
    let lifetime_error_result = holder.function_returning_boxstream_with_lifetime_error().await;
