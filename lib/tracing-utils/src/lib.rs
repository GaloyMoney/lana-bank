...
/// Follows the same pattern as JobPollerHandle for consistency
#[derive(Clone)]
...
pub fn init_tracer(config: TracingConfig) -> anyhow::Result<()> {
    ...
    // Store handle in global state for graceful shutdown (similar to JobPollerHandle pattern)
...
///
/// This follows the same pattern as job::shutdown() for consistency.
...
#[instrument(name = "tracing.shutdown", skip_all)]
pub fn shutdown_tracer() -> Result<(), TracingError> {
    ...
    if let Some(handle) = handle {
        ...
        warn!("No tracer handle found during shutdown");
    }

    Ok(())
}

#[instrument(name = "tracing.perform_shutdown", skip(handle))]
fn perform_shutdown(handle: TracerHandle) -> Result<(), TracingError> {
    // Ensure shutdown is only called once (like JobPollerHandle pattern)
    if handle
        .shutdown_called
        .compare_exchange(false, true, Ordering::SeqCst, Ordering::SeqCst)
        .is_err()
    {
        warn!("Tracer shutdown already called, skipping");
        return Ok(());
    }
    ...
    info!("Tracer provider flushed successfully");
